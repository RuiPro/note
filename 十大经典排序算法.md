...
虽然和归并排序类似，但快速排序是动态分配临界的(归并排序的迭代法是瓜分到最小元素)，而且针对不同数据其比较次数也没有很明确的计算方法确定其比较次数和位置，因此迭代法是非常难以实现的。但是只用递归非常容易导致爆栈，有什么办法可以既使用递归，又不大量占用栈空间呢？有，那就是在堆区自定义一个栈用于递归。下个小结将详细说明自定义栈与递归。
