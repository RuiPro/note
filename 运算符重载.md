...
- 对赋值运算符`=`进行重载时，需要考虑浅拷贝和深拷贝的问题。即：C++对赋值运算符的操作是值传递，这对一般变量而言是完全可行的，如`int i = j`。但是对于参数是指针而言，尤其是对于指向堆区数据的指针而言，简单的值传递会导致在进行赋值操作时，编译器会将指针值进行传递，进而导致多个指针指向了一个内存空间，产生内存非法访问的问题。因此在重载赋值运算符`=`时，需要考虑使用深拷贝解决这个问题。
